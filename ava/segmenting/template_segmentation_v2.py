...
def segment_file(filename, template, p):
	...
	mad_sigma = 1.4826 * np.median(abs_devs) + EPSILON

	# plt.plot(dt*np.arange(len(result)), result)
	# plt.axhline(y=median)
	# plt.axhline(y=median+2*mad_sigma)
	# plt.title(filename)
	...
	indices = np.argwhere(result>median + mad_sigma*NUM_SIGMA).flatten()[1:-1]
	max_indices = []
	...
	max_indices = clean_max_indices(max_indices, times, result)
