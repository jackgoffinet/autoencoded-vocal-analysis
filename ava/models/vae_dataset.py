...
class WarpedWindowDataset(Dataset):
	...
	# def make_inverse_knots(self):
	# 	""" """
	# 	self.x_inv_knots = np.zeros_like(self.x_knots)
	# 	self.y_inv_knots = np.zeros_like(self.y_knots)
	# 	n = self.x_knots.shape[1]
	# 	for k in range(len(self.x_inv_knots)):
	# 		# Fix the first knot.
	# 		x1 = self.y_knots[k,0]
	# 		y1 = self.x_knots[k,0]
	# 		x2 = self.y_knots[k,1]
	# 		y2 = self.x_knots[k,1]
	# 		m = (y2 - y1) / (x2 - x1)
	# 		self.x_inv_knots[k,0] = 0.0
	# 		self.y_inv_knots[k,0] = y1 + m * (-x1)
	# 		# Fix the middle knots.
	# 		for i in range(1,n-1):
	# 			self.x_inv_knots[k,i] = self.y_knots[k,i]
	# 			self.y_inv_knots[k,i] = self.x_knots[k,i]
	# 		# Fix the last knot.
	# 		x1 = self.y_knots[k,-2]
	# 		y1 = self.x_knots[k,-2]
	# 		x2 = self.y_knots[k,-1]
	# 		y2 = self.x_knots[k,-1]
	# 		m = (y2 - y1) / (x2 - x1)
	# 		self.x_inv_knots[k,-1] = 1.0
	# 		self.y_inv_knots[k,-1] = y1 + m * (1.0 - x1)
