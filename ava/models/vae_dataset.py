...
class WarpedWindowDataset(Dataset):
	...
	def __init__(self, audio_filenames, template_dir, p, \
		transform=None, dataset_length=2000, load_warp=False):
		"""
		Create a torch.utils.data.Dataset for chunks of animal vocalization.
		...
		self.p = p
		self.transform = transform
		self.linear_warp(template_dir, load_warp=load_warp)
		# self.dynamic_warp(template_dir, load_warp=load_warp)
	...
	# def make_loss_func(self, template, spec):
	# 	"""Make a time warping loss function."""
	# 	def loss_func(coeffs):
	# 		target_ts = coeffs[0] + coeffs[1] * np.arange(template.shape[1])
	# 		interp = interp2d(np.arange(spec.shape[1]), \
	# 			np.arange(spec.shape[0]), spec, bounds_error=False, \
	# 			fill_value=self.p['spec_min_val'])
	# 		interp_spec = interp(target_ts, np.arange(spec.shape[0]), \
	# 			assume_sorted=True)
	# 		return np.sum(np.power(template - interp_spec, 2))
	# 	return loss_func


	# def linear_warp(self, template_dir, load_warp=False):
	...
	# 	if load_warp:
	# 		try:
	# 			warps = np.load('temp_data/warps.npy', allow_pickle=True).item()
	...
	# 	warps = {}
	# 	for i in range(len(self.audio)):
	# 		spec, _ = self.get_spec(self.audio[i])
	# 		loss_func = self.make_loss_func(template, spec)
	# 		x0 = np.array([0.0,1.0])
	# 		res = minimize(loss_func, x0, bounds=[(-.1,.1),(.8,1.2)])
	# 		self.coeffs[i] = res.x[:]
	# 		warps[self.audio_filenames[i]] = self.coeffs[i]
	# 	np.save('temp_data/warps.npy', warps)

	def linear_warp(self, template_dir, load_warp=False):
		"""
		Linearly warp each song rendition to the template.

		densewarp(self.y_knots, self.x_knots, X, np.empty_like(X))
		"""
		template = self.get_template(template_dir)
		self.coeffs = np.zeros((len(self.audio), 2))
		if load_warp:
			try:
				self.warp_model = np.load('temp_data/warp_model.npy', allow_pickle=True).item()
				return
			except IOError:
				pass
		amp_traces = []
		...
		model = PiecewiseWarping(n_knots=1, warp_reg_scale=1e-6, \
		...
		plt.close('all')
		K = model.x_knots.shape[0]
		# T, N = model.template.shape
		T, N = 84, 1
		unwarped = densewarp(model.y_knots, model.x_knots, amp_traces, np.empty((K, T, N)))
		...
		# print(model.x_knots.shape) # 2413, 2
		# print(model.y_knots.shape) # 2413, 2
		# print(np.min(model.x_knots), np.max(model.x_knots)) # 0,1
		# print(np.min(model.y_knots), np.max(model.y_knots)) # -0.0X, 1.X
		x_knots, y_knots = model.x_knots, model.y_knots
		model = {}
		for i, fn in enumerate(self.audio_filenames):
			model[fn] = {'y': y_knots[i], 'x': x_knots[i]}
		np.save('temp_data/warp_model.npy', model)
		self.warp_model = model



	# def dynamic_warp(self, template_dir, load_warp=False):
	# 	"""
	# 	Dynamic time warping for birdsong.
	#
	# 	NOTE: UNFINISHED
	# 	"""
	# 	template = self.get_template(template_dir)
	# 	if load_warp:
	# 		try:
	# 			self.warps = np.load('temp_data/dynamic_warps.npy', \
	...
	# 	template = template.T
	# 	for i in range(len(self.audio)):
	# 		spec = self.get_spec(self.audio[i])[0].T
	# 		res = dtw(spec, template, window_type="itakura")
	# 		warping_path = res.get_warping_path(target='query')
	# 		warps[self.audio_filenames[i]] = warping_path
	...
	def __getitem__(self, index, seed=None):
		...
		for i in index:
			while True:
				...
				# Then choose a chunk of audio uniformly at random.
				start_t = np.random.rand() * (1.0 - self.window_frac)
				stop_t = start_t + self.window_frac
				target_ts = np.linspace(start_t, stop_t, self.p['num_time_bins'])
				# Inverse warp.
				x_knot = self.warp_model[self.audio_filenames[file_index]]['x']
				y_knot = self.warp_model[self.audio_filenames[file_index]]['y']
				start_wt = (start_t - y_knot[0]) * self.template_dur / y_knot[1]
				stop_wt = (stop_t - y_knot[0]) * self.template_dur / y_knot[1]
				target_ts = np.linspace(start_wt, stop_wt, self.p['num_time_bins'])
				# Then make a spectrogram.
				spec, flag = self.p['get_spec'](start_wt, stop_wt, \
					self.audio[file_index], self.p, fs=self.fs, \
	...
	# 			# Then make a spectrogram.
	# 			spec, flag = self.p['get_spec'](start_t, stop_t, \
	# 				self.audio[file_index], self.p, fs=self.fs, \
	# 				max_dur=stop_t-start_t)
	# 			if not flag:
	# 				continue
	# 			if self.transform:
	# 				spec = self.transform(spec)
	...
	def get_specific_item(self, query_filename, quantile):
		...
		# start_t = quantile * (self.template_dur - self.p['window_length'])
		# stop_t = start_t + self.p['window_length']
		# # Warp to the current audio chunk.
		# start_t = self.coeffs[file_index,0] + self.coeffs[file_index,1] * start_t
		# stop_t = self.coeffs[file_index,0] + self.coeffs[file_index,1] * stop_t
		# # Then make a spectrogram.
		# spec, flag = self.p['get_spec'](start_t, stop_t, \
		# 	self.audio[file_index], self.p, fs=self.fs, max_dur=stop_t-start_t)

		start_t = quantile * (1.0 - self.window_frac)
		stop_t = start_t + self.window_frac
		# Inverse warp.
		y_knot = self.warp_model[self.audio_filenames[file_index]]
		start_wt = (start_t - y_knot[0]) * self.template_dur / y_knot[1]
		stop_wt = (stop_t - y_knot[0]) * self.template_dur / y_knot[1]
		target_ts = np.linspace(start_wt, stop_wt, self.p['num_time_bins'])
		# Then make a spectrogram.
